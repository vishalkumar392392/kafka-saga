package com.eventcart.orders.config;
...
//	}
	...
	@Bean
	NewTopic ordersEventTopicName() {
		return TopicBuilder.name(ordersEventTopicName).partitions(3).replicas(3)
				.configs(Map.of("min.insync.replicas", "2")).build();
	}
	
	@Bean
	NewTopic productsCommandTopicName() {
		return TopicBuilder.name(productsCommandTopicName).partitions(3).replicas(3)
				.configs(Map.of("min.insync.replicas", "2")).build();
	}
	
	@Bean
	NewTopic processPaymentCommandTopicName() {
		return TopicBuilder.name(paymentsCommandTopicName).partitions(3).replicas(3)
				.configs(Map.of("min.insync.replicas", "2")).build();
	}
	
	@Bean
	NewTopic ordersCommandTopicName() {
		return TopicBuilder.name(ordersCommandTopicName).partitions(3).replicas(3)
				.configs(Map.of("min.insync.replicas", "2")).build();
	}
